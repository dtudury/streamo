 ...
 */
async function callMiddleware (mw, { method = 'GET', path = '/', headers = {} } = {}) {
  const state = {
    statusCode: 200,
    headers: {},
    body: null,
  ...
  await mw(req, res, () => { state.nextCalled = true })
  return state
}
...
describe(import.meta.url, ({ test }) => {
  ...
  test('serves index.html for /', async ({ assert }) => {
  ...
  test('falls through (next) when path not in files map', async ({ assert }) => {
    const repo = makeRepo({ files: { 'index.html': '<p>here</p>' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/missing.html' })
  ...
  test('falls through when files key missing from value', async ({ assert }) => {
  ...
  test('falls through for non-GET/HEAD methods', async ({ assert }) => {
    const repo = makeRepo({ files: { 'index.html': 'x' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { method: 'POST', path: '/' })
    assert.equal(r.nextCalled, true)
  })

  test('rejects path traversal', async ({ assert }) => {
    const repo = makeRepo({ files: { 'index.html': 'x', '../secret.html': 'y' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/../secret.html' })
    assert.equal(r.nextCalled, true)
  })

  // ── content types ────────────────────────────────────────────────────────

  test('JS files served as text/javascript', async ({ assert }) => {
    const repo = makeRepo({ files: { 'app.js': 'console.log("hi")' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/app.js' })
    assert.equal(r.headers['content-type'], 'text/javascript; charset=utf-8')
  })

  test('SVG served as image/svg+xml', async ({ assert }) => {
    const repo = makeRepo({ files: { 'icon.svg': '<svg/>' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/icon.svg' })
    assert.equal(r.headers['content-type'], 'image/svg+xml')
  })

  test('unknown extension served as octet-stream', async ({ assert }) => {
    const repo = makeRepo({ files: { 'thing.bin': new Uint8Array([1, 2, 3]) } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/thing.bin' })
    assert.equal(r.headers['content-type'], 'application/octet-stream')
  })

  test('binary Uint8Array passed through unchanged', async ({ assert }) => {
    const bytes = new Uint8Array([0x89, 0x50, 0x4e, 0x47])  // PNG magic
    const repo = makeRepo({ files: { 'logo.png': bytes } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/logo.png' })
    assert.equal(r.headers['content-type'], 'image/png')
    assert.equal(r.body[0], 0x89)
    assert.equal(r.body[1], 0x50)
  })

  test('JSON objects re-serialized to pretty JSON', async ({ assert }) => {
    ...
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { path: '/config.json' })
    assert.equal(r.statusCode, 200)
  ...
  test('sets a strong ETag derived from content', async ({ assert }) => {
  ...
  test('returns 304 when If-None-Match matches', async ({ assert }) => {
    const repo = makeRepo({ files: { 'index.html': '<p>x</p>' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const first = await callMiddleware(mw, { path: '/' })
    const etag = first.headers.etag
    const second = await callMiddleware(mw, { path: '/', headers: { 'if-none-match': etag } })
    assert.equal(second.statusCode, 304)
    assert.equal(second.body, null)
  })

  test('ETag changes when repo content changes', async ({ assert }) => {
    const repo = makeRepo({ files: { 'index.html': '<p>v1</p>' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const before = (await callMiddleware(mw, { path: '/' })).headers.etag
    const working = repo.checkout()
    working.set({ files: { 'index.html': '<p>v2</p>' } })
    repo.commit(working, 'update')
    const after = (await callMiddleware(mw, { path: '/' })).headers.etag
    assert.notEqual(before, after)
  })

  // ── importmap injection ─────────────────────────────────────────────────

  test('injects importmap into HTML <head>', async ({ assert }) => {
    const repo = makeRepo({
  ...
  test('skips importmap injection on non-HTML responses', async ({ assert }) => {
  ...
  test('respects existing importmap in document', async ({ assert }) => {
  ...
  test('importmap can be disabled', async ({ assert }) => {
    const repo = makeRepo({
  ...
  test('importmap libraryPath is configurable', async ({ assert }) => {
  ...
  test('HEAD returns headers only', async ({ assert }) => {
    const repo = makeRepo({ files: { 'index.html': '<h1>x</h1>' } })
    const mw = serveFromRepo(repo, { injectImportMap: false })
    const r = await callMiddleware(mw, { method: 'HEAD', path: '/' })
    assert.equal(r.statusCode, 200)
    assert.equal(r.headers['content-type'], 'text/html; charset=utf-8')
  ...
  test('mounts: resolves files-first on the served repo', async ({ assert }) => {
  ...
  test('mounts: serves through a single mount to another record', async ({ assert }) => {
    ...
    const r = await callMiddleware(mw, { path: '/lib/foo.js' })
    assert.equal(r.statusCode, 200)
    assert.equal(r.headers['content-type'], 'text/javascript; charset=utf-8')
    assert.equal(bodyAsString(r.body), 'console.log("from B")')
  })

  test('mounts: longest matching prefix wins', async ({ assert }) => {
    const a = makeRepo({
      files: {},
    ...
    const r1 = await callMiddleware(mw, { path: '/lib/x.js' })
    assert.equal(bodyAsString(r1.body), 'B-version')
    // /lib/v2/x.js matches BOTH `lib/` and `lib/v2/` — the longer one wins
    const r2 = await callMiddleware(mw, { path: '/lib/v2/x.js' })
    assert.equal(bodyAsString(r2.body), 'C-version')
  })

  test('mounts: missing file in mounted record falls through (404)', async ({ assert }) => {
    ...
    const r = await callMiddleware(mw, { path: '/lib/missing.js' })
    assert.ok(r.nextCalled, 'expected next() — missing file in mount falls through')
  })

  test('mounts: missing mount target (not in registry) falls through', async ({ assert }) => {
    const a = makeRepo({ files: {}, mounts: { 'lib/': { key: KEY_B } } })
    ...
    const r = await callMiddleware(mw, { path: '/lib/anything' })
    assert.ok(r.nextCalled)
  })

  test('mounts: cycle (A → B → A) returns 404, not infinite recursion', async ({ assert }) => {
    ...
    const r = await callMiddleware(mw, { path: '/b/a/anything' })
    assert.ok(r.nextCalled)
  })

  test('mounts: self-mount (A → A) returns 404 on the would-loop path', async ({ assert }) => {
    ...
    const r1 = await callMiddleware(mw, { path: '/top.js' })
    assert.equal(bodyAsString(r1.body), 'self')
    // But /me/top.js attempts to recurse into A again → cycle → 404
    const r2 = await callMiddleware(mw, { path: '/me/top.js' })
    assert.ok(r2.nextCalled)
  })

  test('mounts: nested mount-through-mount works (A → B → C)', async ({ assert }) => {
    const a = makeRepo({ files: {}, mounts: { 'b/': { key: KEY_B } } })
    ...
    const r = await callMiddleware(mw, { path: '/b/c/deep.txt' })
    assert.equal(r.statusCode, 200)
    assert.equal(bodyAsString(r.body), 'three levels')
  })

  test('mounts: pinned dataAddress reads the record at that specific commit', async ({ assert }) => {
