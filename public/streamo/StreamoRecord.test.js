...
describe(import.meta.url, ({ test }) => {
  ...
  test('merge into empty target: pure-copy fork — no parent, has remoteParent', async ({ assert }) => {
    const source = new WritableStreamoRecord()
  ...
  // ── recoveryStuck: the substrate-articulated "auto-resolve gave up" ──

  test('recoveryStuck is null on a fresh Writable Record', async ({ assert }) => {
    const repo = new WritableStreamoRecord()
    assert.equal(repo.recoveryStuck, null)
  })

  test('recoveryStuck is reactive — watchers fire when it sets/clears', async ({ assert }) => {
    const repo = new WritableStreamoRecord()
    let runs = 0
    repo.recaller.watch('recoveryStuck-watcher', () => {
      void repo.recoveryStuck
      runs++
    })
    assert.equal(runs, 1, 'initial read fires the watcher once')
    repo._setRecoveryStuck({ attempts: 4, pushRejected: null })
    await new Promise(resolve => setTimeout(resolve, 10))
    assert.equal(runs, 2, 'set fires the watcher')
    repo._setRecoveryStuck(null)
    await new Promise(resolve => setTimeout(resolve, 10))
    assert.equal(runs, 3, 'clear fires the watcher')
  })

  test('_reset clears recoveryStuck', async ({ assert }) => {
    const repo = new WritableStreamoRecord()
    repo._setRecoveryStuck({ attempts: 4 })
    assert.ok(repo.recoveryStuck)
    repo._reset()
    assert.equal(repo.recoveryStuck, null, '_reset is the "throw it all away" verb — clears the stuck flag too')
  })

  test('update exhausting retries with no session sets recoveryStuck', async ({ assert }) => {
    // No session attached → first failure inside the retry loop exits
    // (no path to retry without a session). We force a failure by pre-
    // setting conflictDetected so _awaitChainHash rejects immediately.
    const { Signer } = await import('./Signer.js')
    const signer = new Signer('alice', 'pass', 1)
    await signer.keysFor('rs-test')

    const repo = new WritableStreamoRecord()
    repo.attachSigner(signer, 'rs-test')
    // Pre-arm the conflict so _awaitChainHash rejects on its first read.
    repo._setConflictDetected({ dataAddress: 0 })
    ...
    await repo.update(
      ...
      {
        retries: 0,
        onConflict: state => { onConflictArgs = state }
      }
    )

    assert.ok(onConflictArgs, 'onConflict was invoked when retries exhausted')
    assert.equal(onConflictArgs.attempts, 1, 'one attempt was made before giving up')
    assert.ok(repo.recoveryStuck, 'recoveryStuck fired alongside onConflict')
    assert.equal(repo.recoveryStuck.attempts, 1, 'recoveryStuck shape matches onConflict shape')
  })

  test('recoveryStuck clears on a fresh update call (the "retry now" semantic)', async ({ assert }) => {
    // The substrate's retry verb IS `repo.update(() => userChoice)`.
    // A fresh call clears the stuck cell on entry — the resolve UI
    // in the app naturally hides. If the new attempt succeeds the
    // cell stays clear; if it also exhausts, the cell sets again.
    const repo = new WritableStreamoRecord()
    repo._setRecoveryStuck({ attempts: 3 })
    assert.ok(repo.recoveryStuck, 'precondition: cell is set')

    let runs = 0
    repo.recaller.watch('clear-watcher', () => {
      void repo.recoveryStuck
      runs++
    })
    assert.equal(runs, 1)

    // Trigger the entry-clear by starting an update. We don't need it
    // to complete — we just need to observe the clear-on-entry. The
    // simplest way: a stream WITH a signer but WITHOUT a session, so
    // update has nothing to await against. Use a short retries to
    // bound the test if anything unexpected happens.
    //
    // Pre-arm a conflict so the first awaitChainHash rejects fast.
    const { Signer } = await import('./Signer.js')
    const signer = new Signer('alice', 'pass', 1)
    await signer.keysFor('rs-retry')
    repo.attachSigner(signer, 'rs-retry')
    repo._setConflictDetected({ dataAddress: 0 })

    await repo.update(() => ({ x: 1 }), { retries: 0, onConflict: () => {} })

    // Cell flow: cleared at update entry (runs=2), then re-set when
    // retries exhaust (runs=3). At least one of those fires.
    await new Promise(resolve => setTimeout(resolve, 10))
    assert.ok(runs >= 2, `recoveryStuck mutated during update (runs=${runs})`)
    // Final state: re-set, because update exhausted again.
    assert.ok(repo.recoveryStuck, 're-set on the new exhaustion')
  })
})
