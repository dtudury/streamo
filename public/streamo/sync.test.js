...
async function openSigned (registry) {
  ...
  // _materialize returns a Mirror; authoring goes through `.local`.
  const mirror = await registry._materialize(key)
...
describe(import.meta.url, ({ test }) => {
  test('outlet syncs existing stream data to a new origin', async ({ assert }) => {
    ...
    const clientStream = (await clientRegistry._materialize(await ensureKey())).local
  ...
  test('origin syncs local data up to the outlet', async ({ assert }) => {
    ...
    const ws = await originSync(clientStream, KEY, `localhost:${port}`)
    const serverStream = (await serverRegistry._materialize(KEY)).local
